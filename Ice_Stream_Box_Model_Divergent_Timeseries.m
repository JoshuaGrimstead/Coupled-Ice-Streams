%% Divergent Topology Timeseries (Grimstead et al., ,2026)
...
%% Set parameters
load("p_base_values.mat") % load base conditions
% other variables easily tested. Base conditoions result in periodic
% behaviour

% p.L2 = 215e3;
 p.L1 = 62.8e3; %chosen to demonstrate chaotic behaviour
% p.L3 = 250e3;
% 
 p.L1f =   p.L1 ;
% p.L2f =   p.L2 ;
...
% 
% 
% p.htmax1 = 1;           % maximal till height
% p.htmax2 = 2;
% p.htmax3 = 2;
...
t_final = 20e4;
%%
% p.T_s1 = 15;      %ice surface temperature (negative)
 p.T_s2 = 14.02;
% p.T_s3 = 15; 
% 
% p.T_s1f = p.T_s1 ; 
...
slope12 = h2/p.L2; 
slope13 = h3/p.L3; 
slope1 = (p.W2/p.W1*(h1-h2) + p.W3/p.W1*(h1-h3))/p.L1;

% driving, frictional stresses and basal velocities
tau_d1 = p.rho_i*p.g*h1.*(slope1);        %calculate driving stress
...
tau_d2 = p.rho_i*p.g*h2.*(slope12);        %calculate driving stress
...
tau_d3 = p.rho_i*p.g*h3.*(slope13);        %calculate driving stress
...
nexttile(t1);
hold on;
%plot(time./p.year, T(:,2),'k','linewidth',2);
