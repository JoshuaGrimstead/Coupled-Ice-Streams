function rhs=Ice_Stream_Box_Model_RHS_Divergent(t,X,p)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Divergent Topology Ice Stream Box Model (Grimstead et al., ,2026)
% This is the dynamics function for the divergent ice stream box model 
% described in the MSc dissertation of Joshua Grimstead and subsequently
% Grimstead et al., ,2026. This code is a revised version of the code used
% in Kypke et al., APS, 2026. 
...
% well)
%
% See Ice_Stream_Box_Model_Divergent_Timeseries.m for model driver.
%
...
%
% Divergent Coupling Model by Joshua Grimstead, last updated August, 2026
...
tau_d1 = rho_i*g*h1*(slope1);
tau_f1 = p.tau0*exp(-c*e1);         %calculate basal shear stress. JG: note that tau_0 relies on e_c, so e_c can't be correctly defined boxwise in this set up
...
tau_d2 = rho_i*g*h2*(slope12);       %calculate driving stress
...
tau_d3 = rho_i*g*h3*(slope13);        %calculate driving stress
...
%% Defining Box 1 to Box 2,3 Flux

Qf = h1*U1; %box 1 outflow requires only the contens of the box, slopes are accounted for in driving stress -> basal belcoity 
